...
namespace ImprovedShipper
{
    public partial class ShippingRequest : System.Web.UI.Page
    {
        ...
        private void GetDepartments()//Modified byt Dan Engle to handle new dept ddl and limit the use of the db connections to when the method us used. Old sql connection were left open causing connection errors 
        {
            ...
            using (myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["ImprovedShipper.Properties.Settings.OAI_ShippingRequestConnectionString"].ConnectionString))//Added by Dan to replace above connection
            {
                try
                {
                    ...
                    SqlDataAdapter adapter = new SqlDataAdapter("SELECT Department_ID, Department_Name FROM Shipping_Department WHERE Department_Type = 'U' ORDER BY Department_Name", myConnection);//12/17/2018 Ellen advised dept ordered alphabetically
                                           ...
                                           //developer to modify front in ddl of more than 200+ items. Now updating countries can be done from mgmt studio to dynamically populate ddl as needed
        {
            ...
            var All_ISO3166_Countries = (from x in db.ISO3166_Countries
                                         ...
                                         select new
                                         {
                                             Code = x.IATA_Code,
                                             Name = x.ISO3166_Name
                                         }).ToList().OrderBy(x => x.Name);//querys the countries from db and orders by name alphabetically
            ddlCountries.Items.Add(new ListItem("United States", "US"));//12/17/2018 Ellen advised US should be 1st on list
            foreach (var y in All_ISO3166_Countries)//iterated through query list
            {
                ...
                if(y.Code == "US")//we already inserted US to skip
                {
                    continue;
                }
                else//adds additional countries to list alphabetically
                {
        ...
        protected void ddlCountries_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlCountries.SelectedValue == "US" || ddlCountries.SelectedValue == "MX")//US and Mexico have states and districts, UPS requires US addresses contain state
            {
            ...
            else if (ddlCountries.SelectedValue == "CA" || ddlCountries.SelectedValue == "AU" || ddlCountries.SelectedValue == "CN")//Canada, Austrailia, China have provinces and distrincts, UPS requires CA addresses have province
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (ValidPage == true)
            {
                ...
                {
                    if (ddlStates.SelectedIndex != 0)//if user selected a state they hidden element will be set to the text value to be accessible in the message body string
                    {
                        txtState.Text = ddlStates.SelectedItem.Text;
                    }
                    else if (ddlStates.SelectedIndex == 0)//if user left state default email will show message
                    {
                        txtState.Text = "No item selected";
                    }
                    ...
                    MailAddress to = new MailAddress("eduffy@oai.aero");//needs to be changed later to MgrVal
                                        ...
                                        <ul style='list-style-type:square'>
                                            ...
                                            "<li>" +
                                                ...
                                                "<address style='font-family:arial;'>" +
                                                    ...
                                                    "State or Province: " + txtState.Text + "</br>" +
                                            ...
                                            "</li>" +
                                            "<li>" +
                                                "<h4>Ship by date and time: " + calShipByDate.SelectedDate.ToShortDateString() + " at " + ddlShipByTime.SelectedItem.Text + "</h4>" +
                                            "</li>" +
                                        "</ul>" +
                                   "</body>" +
                                "</html>";
